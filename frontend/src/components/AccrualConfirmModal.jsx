...
export const AccrualConfirmModal = ({ isOpen, onClose, onConfirm, isLoading }) => {
  ...
  // We use createPortal to safely render the modal outside of the Navbar's stacking context,
  // preventing it from being trapped, clipped, or hidden by parent sticky/blur layers.
  const modalContent = (
    <div className="fixed inset-0 z-[9999] overflow-y-auto bg-black/80 backdrop-blur-sm">
      
      {/* min-h-screen guarantees full height, pt-[10vh] anchors from top preventing clipping while simulating vertical center */}
      <div className="flex min-h-screen items-start justify-center p-4 pt-[15vh] pb-12">
        
        <div className="relative w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl text-slate-100">
